...
/**
 * A Pyramid Scheme is a particular form of LinkedTree
...
public class PyramidScheme extends LinkedTree<Person> {
    /**
     * Price to recuit people 
     */
    private double recruitPrice;

    /**
     * The amount people paid lol
     */
     ...
     * @param startNode the root of the scheme (Node)
     * @param rPrice    Price to recruit in this scheme
     * @param pUp       Fraction of earnings paid upwards in this
     *                  scheme
     */
    PyramidScheme(MultiTreeNode<Person> startNode,
    ...
    /**
     * Initiates the recursive findNodeChain method to return a
     * list of all nodes that are above the node containing the
     * target. This overload takes a Person.
     *
     * @param sucker the individual from whom others above are
     *               benefitting
     * @return an ArrayList of individuals higher up in the
     *         hierarchy
     */
     ...
     * Initiates the recursive findNodeChain method to return a
     * list of all nodes that are above the node containing the
     * target. This overload takes a MultiTreeNode.
     *
     * @param sucker the individual from whom others above are
     *               benefitting
     * @return an ArrayList of individuals higher up in the
     *         hierarchy
     */
    public ArrayList<MultiTreeNode<Person>> whoBenefits(MultiTreeNode<Person> sucker) {
        return findNodeChain(root, sucker.getElement());
    }

    /**
     * TODO Similar to findNode, but rather than returning the node
     * that contains the target, instead returns a list of nodes
     * above the one that contains the target, in low to high
     * level order.
     *
     * @param node   the current node being examined
     * @param target the Person being searched for
     * @return an ArrayList of nodes that are directly above the
     *         target in the hierarchy
     */
    private ArrayList<MultiTreeNode<Person>> findNodeChain(MultiTreeNode<Person> node,
            Person target) {

        ArrayList<MultiTreeNode<Person>> ascendants = new ArrayList<MultiTreeNode<Person>>(); // create array for target
    ...
    /**
     * Similar to findNode, this will see it the target exists
     * and return a truth value.
     *
     * @param node   the current node being examined
     * @param target the person being searched for
    ...
    /**
     * An override of LinkedTree's addChild, which does the same
     * thing, but ensures the appropriate payment between the
     * recruit and the recruiter, as well as those above them in
     * the hierarchy.
     *
     * @param recruiter the person who is recruiting the other
     *                  into the scheme
     * @param recruit   the person being brought into the scheme
     * @throws ElementNotFoundException element does not exist 
     */
    @Override
    ...
    /**
     * Performs all the transactions inherent to the system
     * wherein a percentage of anything earned by an individual
     * is passed upwards in the hierarchy, such that each
     * individual directly above gets an increasingly smaller
    ...
    /**
     * A recursive method that determines the punishment received
     * by different individuals in the pyramid scheme after it is
     * discovered and halted. Also severs each node from its
     * children, such that no hierarchy exists at all when the
     * collapse is complete.
     *
     * @param personNode the individual currently being examined
     * @return the number of individuals punished
     */
